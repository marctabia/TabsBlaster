...
public class Sound {
    ...
    private void loadSounds() {
        ...
        for (int i = 0; i < shots.length; i++) {
            shots[i] = new AudioNode(assetManager, 
                    "Sounds/shoot-0"+(i+1)+".wav");
        ...
        for (int i = 0; i < explosions.length; i++) {
            explosions[i] = new AudioNode(assetManager, 
                    "Sounds/explosion-0"+(i+1)+".wav");
        ...
        for (int i = 0; i < spawns.length; i++) {
            spawns[i] = new AudioNode(assetManager, 
                    "Sounds/spawn-0"+(i+1)+".wav");
