...
public class ParticleManager {
    ...
    public ParticleManager(Node particleNode, Node guiNode, 
            Spatial standardParticle, Spatial glowParticle, 
    ...
    public void enemyExplosion(Vector3f position) {
        ...
        for (int i=0; i<120; i++) {
            ...
            particle.addControl(
                    new ParticleControl(velocity, 3100, color, 
                    screenWidth, screenHeight));
            particleNode.attachChild(particle);
        }
    }
    
    // we want the direction to be random but not speed
    ...
    // helper function to generate RGBS from HSV, since we want colors
    // that are neon-like; so we set random hue but fixed saturation
    // and value
    ...
    public void blackHoleExplosion(Vector3f position) {
        ...
        for (int i=0; i<numParticles; i++) {
            ...
            Vector3f velocity = Main.getVectorFromAngle(alpha)
    ...
    public void makeExhaustFire(Vector3f position, float rotation) {
        ...
        Vector3f direction = Main.getVectorFromAngle(rotation);
        ...
        Vector3f pos = position.add(Main.getVectorFromAngle(rotation)
                .multLocal(-25f));

        //middle stream
        Vector3f randVec = Main.getVectorFromAngle(
                new Random().nextFloat() * FastMath.PI * 2);
        ...
        //side streams
        Vector3f randVec1 = Main.getVectorFromAngle(
                new Random().nextFloat() * FastMath.PI * 2);
        
        Vector3f randVec2 = Main.getVectorFromAngle(
                new Random().nextFloat() * FastMath.PI * 2);
       
        Vector3f velSide1 = baseVel.add(randVec1.mult(2.4f)).addLocal(perpVel);
       
        Vector3f velSide2 = baseVel.add(randVec2.mult(2.4f))
                .subtractLocal(perpVel);
